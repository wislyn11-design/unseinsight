...
[출력 구조 및 작성 가이드]
- 프론트엔드 UI 화면 출력을 위해 반드시 아래 모든 항목의 제목 앞에 '### ' 기호를 붙여주십시오. (총평 부분도 반드시 포함할 것)
- 각 섹션의 핵심 단어나 중요한 문장은 **Bold** 처리를 하여 진하게 강조해 주십시오. (총평 내용도 포함)
- 💡 (가장 중요) 가독성을 위해 문장이 마침표('.')로 끝날 때마다 반드시 줄바꿈(Enter)을 한 번 하여 문장 단위로 줄을 띄워주십시오. 
- (중요) 아래에 명시된 섹션만 정확히 그 순서대로 작성하고, 명시되지 않은 섹션이나 제목을 임의로 추가하지 마십시오. 특히 1월부터 12월까지 전체를 나열하는 월별 운세는 절대 작성하지 마십시오.
...
### 올해 전체 운세 
- ${currentYear}올해 사주 원국에 들어오는 기운(${currentYear}년 간지)을 바탕으로, 올해의 전반적인 테마와 흐름을 2~3문단으로 요약해 주십시오.
