...
type OracleStart = Pick<PendingOracleStart, "question" | "cardOrder" | "sessionId">;


const CARD_INDEXES = ORACLE_CARDS.map((_, index) => index);

const EMPTY_READING: ReadingState = { status: "idle", reading: "", summary: "", timing: null };

function parseTiming(value: unknown, sajuConnected: boolean): TimingResult | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const timing = value as Record<string, unknown>;
  if (
    typeof timing.window !== "string"
    || typeof timing.basis !== "string"
    || (timing.confidence !== "낮음" && timing.confidence !== "보통")
  ) return null;
  return {
    window: timing.window.trim(),
    basis: timing.basis.trim(),
...
async function requestOracleInterpretation(
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      const timing = parseTiming(result.timing, result.sajuConnected === true);
      if (!reading) throw new Error("Oracle interpretation is empty");

      return { reading, summary, timing };
    } catch (error) {
      lastError = error;
      if (attempt < 2) {
        onRetry();
  ...
  return (
    ...
    >
      ...
      <span className={`absolute left-4 top-4 rounded-full px-3 py-1.5 text-xs font-black shadow-sm backdrop-blur-sm ${isCurrent ? "bg-white/90 text-[#315fca]" : "bg-[#fff5de]/95 text-[#94631a]"}`}>
        {isCurrent ? "첫 번째 카드 · 현재 마음" : "두 번째 카드 · 미래 흐름"}
      </span>
    </div>
  );
}
...
export default function DailyOracleExperience() {
  ...
  const generateCurrentReading = async (index: number, sessionId = sessionRef.current) => {
    ...
    setCurrentReading({ status: "loading", reading: "", summary: "", timing: null });
    ...
    try {
      const result = await requestOracleInterpretation(
        ...
        () => {
          if (sessionRef.current === sessionId) {
            setCurrentReading({ status: "retrying", reading: "", summary: "", timing: null });
          }
        },
      );
      ...
      if (sessionRef.current === sessionId) {
        setCurrentReading({ status: "error", reading: "", summary: "", timing: null });
      }
    }
  };
  ...
  const generateFutureReading = async (index: number, sessionId = sessionRef.current) => {
    ...
    setFutureReading({ status: "loading", reading: "", summary: "", timing: null });
    ...
    try {
      const result = await requestOracleInterpretation(
        ...
        () => {
          if (sessionRef.current === sessionId) {
            setFutureReading({ status: "retrying", reading: "", summary: "", timing: null });
          }
        },
      );
      ...
      if (sessionRef.current === sessionId) {
        setFutureReading({ status: "error", reading: "", summary: "", timing: null });
      }
    }
  };

  const beginOracle = (start: OracleStart) => {
  ...
  const submitQuestion = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.repeated === true) {
        const previousReadingId = typeof result.previousReadingId === "string" ? result.previousReadingId : "";
        setPendingStart({
          question: nextQuestion,
          cardOrder: nextOrder,
          sessionId,
  ...
  const viewPreviousReading = () => {
    if (!pendingStart?.previousReadingId) return;
    const abandonedSessionId = pendingStart.sessionId;
    const previousReadingId = pendingStart.previousReadingId;
    void fetch("/api/oracle/session", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sessionId: abandonedSessionId, action: "abandon" }),
    }).finally(() => {
      window.location.href = `/fortune/oracle/history/${previousReadingId}`;
    });
  };
     ...
     <FortunePageIntro
      ...
      <div className="mx-auto w-full max-w-6xl px-5 pb-12 pt-3 sm:px-8 sm:pb-14">
        ...
        {!question && (
          <section className="relative mx-auto mt-10 max-w-3xl overflow-hidden rounded-[32px] border border-[#d8e3f6] bg-white px-5 py-8 shadow-[0_24px_70px_rgba(45,74,138,.12)] sm:px-10 sm:py-11">
            ...
            <div className="relative">
              <h1 className="text-2xl font-black tracking-[-.04em] text-[#172a4b] sm:text-3xl">지금 가장 마음에 걸리는 고민은 무엇인가요?</h1>
              <form onSubmit={submitQuestion} className="mt-6">
                <label htmlFor="oracle-question" className="sr-only">지금의 고민</label>
                <textarea
                  id="oracle-question"
                  value={questionDraft}
                ...
                {pendingStart && (
                  <div className="mt-5 rounded-2xl border border-[#ead7ae] bg-[#fff9ec] px-5 py-5" role="alert">
                    <p className="text-base font-black text-[#6f501c]">최근 비슷한 고민으로 점괘를 확인했습니다.</p>
                    ...
                    <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                      {pendingStart.previousReadingId && (
                        <button
                          type="button"
                          onClick={viewPreviousReading}
                          className="rounded-xl bg-white px-5 py-3 text-sm font-extrabold text-[#315fca] ring-1 ring-[#b9c9e8] transition hover:bg-[#f4f7ff]"
                        >
                          이전 점괘 보기
                        </button>
                      )}
                ...
                <Link href="/fortune/oracle/history" className="mt-3 inline-flex text-sm font-extrabold text-[#315fca] transition hover:text-[#2855c3]">
                  지난 고민 점괘 보기 →
                </Link>
              </form>
            </div>
            ...
            <div className="mx-auto mt-10 max-w-5xl text-center">
              ...
              {drawing && (
                <p className="mt-5 animate-pulse text-sm font-extrabold text-[#315fca]" aria-live="polite">
                  {drawing === "current" ? "선택한 첫 번째 카드를 펼치고 있어요…" : "선택한 두 번째 카드를 펼치고 있어요…"}
                </p>
              )}
            </div>
                ...
                {futureReading.status === "success" && futureReading.summary && (
                  <section className="mx-auto mt-7 max-w-4xl rounded-[32px] border border-[#d7e2f5] bg-[linear-gradient(145deg,#f9fbff,#eef4ff)] px-6 py-8 shadow-[0_18px_48px_rgba(45,74,138,.1)] sm:px-10">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <p className="text-sm font-extrabold text-[#315fca]">
                          {futureReading.timing?.sajuConnected ? "사주 흐름과 종합 풀이" : "두 카드 종합 풀이"}
                        </p>
                    ...
                    {futureReading.timing && (
                      <div className="mt-5 rounded-2xl border border-[#d6e1f5] bg-white/85 px-5 py-4">
                        <p className="text-xs font-extrabold text-[#72809a]">
                          {futureReading.timing.sajuConnected ? "흐름이 움직일 가능성이 커지는 시기" : "예상 시기 안내"}
                        </p>
                        <p className="mt-1 text-lg font-black text-[#315fca]">{futureReading.timing.window}</p>
                        <p className="mt-2 text-sm font-medium leading-6 text-[#65738b]">{futureReading.timing.basis}</p>
                      </div>
                    )}
                    <div className="mt-6 flex flex-col gap-3 border-t border-[#dce5f4] pt-6 sm:flex-row sm:justify-end">
                      ...
                      <Link href="/fortune/oracle/history" className="rounded-xl bg-white px-5 py-3 text-center text-sm font-extrabold text-[#315fca] ring-1 ring-[#cfdbf0] transition hover:bg-[#f4f7ff]">지난 점괘 보기</Link>
        ...
        <p className="mx-auto mt-7 max-w-2xl text-center text-xs font-medium leading-5 text-[#929caf]">
          고민 점괘는 마음과 가능성을 정리하기 위한 참고 콘텐츠입니다. 중요한 결정은 실제 상황과 충분한 정보를 함께 살펴보세요.
        </p>
      </div>
    </main>
  );
