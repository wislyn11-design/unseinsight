...
async function loadPrimarySaju(supabase: Awaited<ReturnType<typeof createClient>>, userId: string, today: string) {
  const { data: primaryProfiles, error: profileError } = await supabase
    .from("birth_profiles")
    .select("id, gender, birth_time_unknown")
    .eq("user_id", userId)
    .eq("is_primary", true)
    .limit(1);
  ...
  if (!chart) return { profile, chart: null, context: null };

  return {
    profile,
    chart,
    context: buildOracleTodaySajuContext({
...
async function loadSessionPrerequisites(
  ...
  const [historyResult, primarySaju] = await Promise.all([
    supabase
      ...
      .select("id, question_normalized, question_topic, question_intent, created_at")
      .eq("user_id", userId)
      .eq("draw_date", today)
  ...
  try {
    ...
    const bestMatch = (previousReadings ?? []).reduce<{
      ...
      return !best || score > best.score
        ? { id: row.id, createdAt: row.created_at, score }
        : best;
    }, null);
    ...
    const { data: session, error: insertError } = await withJwtClockRecovery(
      ...
      () => supabase
        ...
        .insert({
          ...
          prompt_version: "oracle-reading-v3-saju-context",
          ...
          today_day_pillar: primarySaju?.context?.today?.dayPillar ?? null,
          today_saju_context: primarySaju?.context ?? null,
          today_context_version: primarySaju?.context
            ? ORACLE_TODAY_SAJU_CONTEXT_VERSION
            : null,
        })
        .select("id")
